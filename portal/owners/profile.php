...
<?php 
error_reporting(0);
    $rows = get_account_details($_SESSION['owners_id']);
    ...
    if(isset($_POST['saveChanges'])) {
        ...
        update_owner_profile($license,$surname,$firstname,$middlename,$email,$contact);
    }
    ...
    if(isset($_POST['btnChangePassword'])) {
        ...
        update_owner_password($old_password,$new_password,$confirm_new_password);
    }
    ...
    if(isset($_GET['buycredits']) == 'status') {
        echo "gasgasgasg";
    }
...
<body class="animsition site-navbar-small ">
        ...
        <div class="page-content">
            <div class="row">
                ...
                <div class="col-md-4 col-12">
                            ...
                            <div class="row">
                                ...
                                <div class="col-12">
                                    ...
                                    <form enctype="multipart/form-data" method="POST">
                                        <div class="form-group">
                                            <input type="file" class="form-control" name="requirement" required>
                                        </div>
                                        <div class="form-group">
                                            <button type="submit" name="upload"
                                                class="btn btn-primary btn-block btn-sm mb-2">Upload Business Permit</button>
                                                </form>
                                                <?php if($rows['Upgrade'] == 2) { echo "<div class='alert alert-success'><strong>Congratulations!</strong> You are now a <b>Macro</b> user</div>";  }else{ ?>
                                                <form action="" method="POST">
                                                <a name="UpgradeMacro" id="UpgradeMacro"
                                                class="btn btn-warning <?php if($rows['Upgrade'] == 1) { echo "disabled"; } ?> text-white btn-block btn-sm mt-3" ><?php if($rows['Upgrade'] == 1) { echo "Waiting for the approval"; }else { echo "Upgrade to Macro";} ?></a>
                                                </form>
                                                <?php }?>
                                        </div>
                    ...
                    <?php if(buy_credits_status($_SESSION['owners_id']) == "" || buy_credits_status($_SESSION['owners_id']) == "Reject" || buy_credits_status($_SESSION['owners_id']) == "None" || buy_credits_status($_SESSION['owners_id']) == "Expired"){?>
                    <div class="panel">
                        <div class="panel-body">
                            <input type="hidden" value="<?= $rows['id']?>" id="userCreditsID">
                            <p><b>GCASH:</b> 09471559441 | Mark Allan Carba<br>Remittance: Mark Allan Carba | 09471559441 | Moalboal Cebu</p>
                            <small>Lorem ipsum dolor sit amet consectetur adipisicing elit. Quae, aspernatur.</small>
                            <div style="width: 100%;"><a id="buyCredits" class="btn bg-warning" style="width: 100%; color: #000; font-weight: 600; ">Buy <?= $rows['Upgrade'] == 2 ?  "1000" :  "500" ?> Credits</a></div>
                            
                        </div>
                    </div>
                        <?php if(buy_credits_status($_SESSION['owners_id']) == "Reject"){ ?>
                            <h5 class="alert alert-danger">We're very sorry, you have been rejected.</h5>
                        <?php }?>
                <?php }else if(buy_credits_status($_SESSION['owners_id']) == "Pending") { ?>
                    <div class='alert alert-warning'><strong>Thank you for purchasing credits!</strong> Please wait, your transaction is under review. </div>
                    <?php }else{?>
                            <h5 class="alert alert-success">Thank you for supporting ADL Car Rental INC</h5>
                        <?php }?>
    ...
    <div class="modal fade" id="subsciptoionModal" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
            aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                    ...
                    <div class="modal-body">
                        <div class="row">
                            <form id="buySubscription" enctype="multipart/form-data">
                            <div class="col-12">
                                <div class="form-group mb-3">
                                    <label for="">Full Name</label>
                                    <input type="text" name="cfname" class="form-control" value="">
                                </div>
                                <div class="form-group mb-3">
                                    <label for="">Contact No.</label>
                                    <input type="text" name="cnumber" class="form-control" value="">
                                    <input id="customerIdinput" type="hidden" name="customerIdinput" class="form-control">
                                ...
                                <div class="form-group mb-3">
                                    ...
                                    <input type="file" name="uploadReceipt" class="form-control">
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button id="submitBuyCredits" type="submit" class="btn btn-primary btn-sm text-white">Submit</button>
                    </div>
                </div></form>
            </div>
        </div>
    ...
    <script>
        ...
        $("#UpgradeMacro").on("click", function(){
            swal({
              title: "Are you sure?",
              text:  "Do you want to upgrade your account to Macro?",
              icon: "warning",
              buttons: true,
              dangerMode: true,
          })
          .then((isOkay) => {
              if (isOkay){
                $.ajax({
                    type: "POST",
                    url: "profile.php",
                    data: {upgradeAccount : "UpgradeMacro"},
        ...
        $("#buyCredits").on("click", function(){
            $("#subsciptoionModal").modal("show");
            var userID = $("#userCreditsID").val();
            $("#customerIdinput").val(userID);
        ...
        $("#buySubscription").submit(function(e){
                e.preventDefault();
