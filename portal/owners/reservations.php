            ...
            <div class="panel">
                ...
                <div class="panel-body">
                    ...
                    <table class="table table-hover dataTable table-striped w-full" data-plugin="dataTable">
                        ...
                        <tbody>
                            <?php $i=1; foreach(get_my_transactions($_SESSION['owners_id']) as $data) { ?>
                                <?php $rdata = get_account_details($data['customer_id']);?>
                                <tr>
                                    <td style="width:1px"><?=$i++?></td>
                                    <td><?=$rdata['firstname'].' '.$rdata['middlename'].' '.$rdata['surname']?></td>
                                    <td><?=$data['destination']?></td>
                                    <td><?=$data['from']?></td>
                                    <td><?=$data['to']?></td>
                                    <td>₱<?=number_format($data['rate_per_day'],2)?></td>
                                    <td><?=$data['days_rented']?></td>
                                    <td>₱<?=number_format($data['total'],2)?></td>
                                    <td><?=$data['reference']?></td>
                                    <td><?=$data['date']?></td>
                                    <td><?=$data['status']?></td>
                                    <td><a href="reservation-details.php?id=<?=($data['id'])?>&customer=<?=($data['customer_id'])?>"><i class="icon wb-pencil"></i></a></td>
                                </tr> 
                            <?php } ?>
                        </tbody>
                    </table>
