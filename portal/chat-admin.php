...
if(isset($_POST['sendChat'])) {
    ...
    if($filename == ""){
        save_chat_to_admin($message,$reference,$_SESSION['customer_id']);
    }else{
        $filename = $_FILES['sendImage']['name'];
        $imageTmpName = $_FILES['sendImage']['tmp_name'];
        $finalname = "cImg-".$filename;
        if(save_chat_to_admin($finalname,$reference,$_SESSION['customer_id'])){
            move_uploaded_file($imageTmpName,'../images/chats/'.$finalname);
        }
    }
...
<body class="animsition site-navbar-small app-message page-aside-scroll page-aside-left">
    ...
    <div class="page">
        ...
        <div class="page-main">
            ...
            <div class="app-message-chats">
                <div class="chats">
                    ...
                    <?php foreach(get_chats($_GET['reference']) as $chat) { ?>
                        <div class="chat <?=$chat['position'] == 0 ? 'chat-left' : ''?>">
                            ...
                            <div class="chat-body">
                                <div class="chat-content">
                                    <?php if(substr($chat['message'],0,5) == "cImg-"){ echo "<img src='../images/chats/".$chat['message']."' style='width='250'>";} else { echo $chat['message']; }?>
                                </div>
                            </div>
            ...
            <!-- Message Input-->
            <form method="POST" class="app-message-input" enctype="multipart/form-data" >
                <input type="hidden" name="reference" value="<?=$_GET['reference']?>">
                <input type="hidden" name="id" value="<?=$_GET['id']?>">

                <div style="display: flex; justify-content: right; flex-direction: row;  height: 40px; position: relative; width: 50%; float: right;">
                    
                    <textarea class="form-control" name="message" rows="1"></textarea>
                    <div style="width: 30px;">
                        <label for="files" class="btn"><i class="fa-solid fa-file" style="font-size: 30px;"></i></label>
                        <input id="files" style="visibility:hidden;" type="file" name="sendImage">
                    </div>
                    <button  class=" btn btn-primary" name="sendChat" type="submit" style="margin-left: 30px;">SEND</button>
